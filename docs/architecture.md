...
| 4 | Continuous Improvement | Per-manager preference store injected into report prompt; system-level golden-bucket loop (§5.4) | Preference injection implemented (local JSON); learning loop design-only |
...
| 7 | Observability | Per-node metrics, Langfuse traces joined by trace ID, alerting (§5.7) | Design-only (structured logging in prototype) |
...
**User level.** Each manager has a preference profile — **Production:** Firestore document per `user_id` · **Prototype:** `data/preferences.json` — holding presentation preferences (`format: table | bullets`, verbosity, favourite metrics). Preferences are written two ways: explicitly ("always give me tables" → the router detects a preference statement and persists it) and implicitly in production (repeated re-format requests observed in interaction logs update the profile via a batch job). `load_context` injects the profile into the report-generation prompt every turn, so Manager A gets tables and Manager B gets bullet points without either asking twice.
...
- **Deterministic unit layer (implemented in prototype):** pytest covers `sql_guard` (denylist, non-SELECT rejection), `pii_mask` (regex and column-sweep cases), report-store scoping, and the interrupt flow with a scripted checkpointer — no LLM needed, runs in milliseconds.
